...
object TextExample extends UIExampleScreen with UIScreen {
  ...
  override protected val drawable = OpenTypeFont.fromURL(GoogleFont.`Open Sans`.`regular`).flatMap { font =>
    Paint.video(History.url().withPath("/sample.mp4"), autoPlay = false).map { videoPaint =>
      RenderTimer(
        Group(
