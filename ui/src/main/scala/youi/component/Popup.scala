...
import rapid.Task
import youi.component.types.{Display, PositionType, UserSelect}
import youi.easing.Easing
import youi.task._
import youi.{Chained, Initializable, ui}
import reactify._
...
class Popup(showGlassPane: Boolean = true) extends Container with Initializable {
  ...
  override protected def initialize(): Unit = {
    ...
    size.width := math.min(preferredWidth, ui.size.width() - 16.0)
    size.height := math.min(preferredHeight, ui.size.height() - 16.0)
