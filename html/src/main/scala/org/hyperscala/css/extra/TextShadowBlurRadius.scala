 ...
 */
class TextShadowBlurRadius(ss: StyleSheetBase) extends Property[Length]()(ss, implicitly[Manifest[Length]]) with DerivedProperty[Length, TextShadow] {
  ...
  def fromT(value: Length) = {
    val ts = other.value
    if (ts == null) {
      TextShadow(blurRadius = value)
    } else {
      ts.copy(blurRadius = value)
    }
  }
