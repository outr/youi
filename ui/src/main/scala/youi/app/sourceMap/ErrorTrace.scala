package youi.app.sourceMap

import fabric.rw._
import youi.stream.StreamURL
import youi.{History, _}
...
//  def toError(event: ErrorEvent): IO[JavaScriptError] = {
...
//  }
  ...
  override def write(record: LogRecord, output: LogOutput, outputFormat: OutputFormat): Unit = if (record.level.value >= Level.Error.value) {
//    val value = record.messages.head.value
//    value match {
...
//            try {
//              val json = JsonParser.parse(jsonString).asInstanceOf[js.Object]
