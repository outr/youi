...
trait PositionSupport extends MeasuredSupport {
  this: Component =>
  ...
  object position {
    ...
    lazy val center: Dep[Double, Double] = Dep(left)(_ + (measured.width / 2.0), _ - (measured.width / 2.0))
    lazy val right: Dep[Double, Double] = Dep(left)(_ + measured.width, _ - measured.width)

    lazy val top: Var[Double] = y
    lazy val middle: Dep[Double, Double] = Dep(top)(_ + (measured.height / 2.0), _ - (measured.height / 2.0))
    lazy val bottom: Dep[Double, Double] = Dep(top)(_ + measured.height, _ - measured.height)
