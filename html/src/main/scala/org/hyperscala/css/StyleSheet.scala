 ...
 */
class StyleSheet private(val map: Map[Style[_], AnyRef] = Map.empty) {
  ...
  def ++(ss: StyleSheet) = new StyleSheet(map ++ ss.map)
  ...
  def set[T](style: Style[T], value: T) = if (value == null) {
    new StyleSheet(map - style)
  } else {
    new StyleSheet(map + (style -> value.asInstanceOf[AnyRef]))
  }
...
object StyleSheet extends ValuePersistence[StyleSheet] with Logging {
  def apply() = new StyleSheet()

  def apply(ss: StyleSheet) = new StyleSheet(ss.map)

  def apply(css: String): StyleSheet = {
    var ss = apply()
    css.split(";").map(s => s.split(":")).map(a => a(0).trim -> a(1).trim).foreach {
      case (key, value) => Style.byCSSName(key) match {
        case Some(style) => ss = ss.set[AnyRef](style.asInstanceOf[Style[AnyRef]], style.persistence.fromString(value, null).asInstanceOf[AnyRef])
