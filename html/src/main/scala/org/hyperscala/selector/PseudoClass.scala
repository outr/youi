...
object PseudoClass extends Enumerated[PseudoClass] {
  ...
  def nthChild(s: String) = PseudoClass(s"nth-child($s)")
  def nthChild(groupedBy: Int, index: Int) = nthChild(s"${groupedBy}n+$index")
  def nthLastChild(s: String) = PseudoClass(s"nth-last-child($s)")
  def nthLastChild(groupedBy: Int, index: Int) = nthLastChild(s"${groupedBy}n+$index")
  def nthLastOfType(s: String) = PseudoClass(s"nth-last-of-type($s)")
  def nthLastOfType(groupedBy: Int, index: Int) = nthLastOfType(s"${groupedBy}n+$index")
  def nthOfType(s: String) = PseudoClass(s"nth-of-type($s)")
  def nthOfType(groupedBy: Int, index: Int) = nthOfType(s"${groupedBy}n+$index")
