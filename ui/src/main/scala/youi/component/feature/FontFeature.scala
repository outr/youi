package youi.component.feature

import youi.component.types.{FontWeight, Prop, SizeProperty}
import youi.font.{GoogleFont, GoogleFontWeight}

import scala.concurrent.{ExecutionContext, Future}

class FontFeature(override val parent: FeatureParent) extends Feature {
  lazy val family: Prop[String] = new Prop[String](parent.css.fontFamily, parent.css.fontFamily_=, parent.measureComponent)
  object weight extends Prop[FontWeight](
    FontWeight.fromString(parent.css.fontWeight).getOrElse(FontWeight.Initial),
    fw => parent.css.fontWeight = FontWeight.toString(fw).getOrElse(""),
    parent.measureComponent
  ) {
    def @=(fontWeight: GoogleFontWeight): Unit = {
      family @= fontWeight.font.family
      this @= FontWeight.fromString(fontWeight.name).getOrElse(FontWeight.Normal)
    }
    def !(fontWeight: Future[GoogleFontWeight])(implicit ec: ExecutionContext): Unit = fontWeight.foreach(fw => this @= fw)
  }
  lazy val size: SizeProperty = new SizeProperty(parent.css.fontSize, parent.css.fontSize_=, callbacks = parent.measureComponent)

  def !(font: Future[GoogleFont])(implicit ec: ExecutionContext): Unit = family ! font.map(_.family)
  def @=(font: GoogleFont): Unit = family @= font.family
}
